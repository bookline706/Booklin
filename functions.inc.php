...
function get_product($con,$limit='',$cat_id='',$product_id='',$search_str='',$sort_order='',$is_best_seller='',$sub_categories=''){
	$sql="select product.*,categories.categories,product_attributes.mrp,product_attributes.price,product_attributes.qty from product_attributes,product,categories where product.status=1 and product_attributes.id=product.id";
...
function productQty($con,$pid){
	$sql="select qty from product_attributes where id='$pid'";
	$res=mysqli_query($con,$sql);
	$row=mysqli_fetch_assoc($res);
	return $row['qty'];
}
